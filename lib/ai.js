...
// The only place model names live. "full" is the production analyst model;
// "cheap" is ~5x cheaper and used by CHEAP_MODE for low-cost dev runs.
export const MODELS = {
 ...
 * @param {Object} options   - Optional: { systemPrompt, model, maxTokens }
...
export async function chat(messages, options = {}) {
  ...
  const response = await client.messages.create({
    model: options.model || MODELS.full,
 ...
 *     model: string (default MODELS.full),
