...
export function buildScores(ctx) {
  ...
  return [
    scoreFor('cand-1', meta, {
      utilitySize: [4, 'Regional utility serving a large population per its own site (ev-1).'],
      nrwEvidence: [4, 'Named non-revenue-water reduction initiative tied to the new reporting rule (ev-1).'],
      regionFit: [5, 'Confirmed operator within the target market.'],
      targetRolePresence: [3, 'Large enough to plausibly have an asset-management function; no named contact yet.'],
      momentumSignal: [3, 'Open funding window is a live, time-bound signal (ev-2).'],
    }),
    scoreFor('cand-2', meta, {
      utilitySize: [2, 'Municipal scope; size not confirmed from available evidence.'],
      nrwEvidence: [2, 'Grant eligibility implies relevance but no named NRW programme (ev-2).'],
      regionFit: [5, 'Confirmed operator within the target market.'],
      targetRolePresence: [1, 'No basis yet to believe a target function exists at this scale — inference only.'],
      momentumSignal: [2, 'Same funding window as cand-1, but weaker fit (ev-2).'],
    }),
  ];
}
      ...
      contacts: [
        {
          ...
          confidence: 0.6,
          uncertainty: 'Title found on a static team page; not confirmed as current.',
        },
      ],
      humanVerificationRequired: [
        'Confirm the utility is actually in scope geographically.',
        'Confirm the grant applies to this organisation type.',
        'Confirm the contact still holds this role before any outreach.',
      ],
      metadata: meta,
    },
  ];
