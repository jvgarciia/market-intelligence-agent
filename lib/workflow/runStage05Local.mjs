 ...
 *   briefAll  boolean — when true, briefs every candidate that survives
 *                       Stage 03 validation instead of the model's curated
 *                       top-pick subset (the default)
...
export async function runStage05Local(runId, options = {}) {
  ...
  try {
    stage05Result = await runStage05(request, candidates, validated, scores, provider, { briefAll: options.briefAll });
  } catch (err) {
    stageFailed = true;
