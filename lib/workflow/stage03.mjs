...
export function buildSystemPrompt() {
  return [
    ...
    '- Judge each underlying fact once and apply that outcome everywhere it',
    '  reappears — a bundled claim restating an already-rejected fact (same',
    '  sources) inherits that rejection; do not let bundling dilute it',
...
export function buildUserMessage(request, stageCtx, sources, signals, candidates, runId, createdAt) {
  ...
  return [
    ...
    '',
    'same-fact consistency: if a signal and a candidate relevanceEvidence claim',
    '(or two candidate claims) restate the same underlying fact using the same',
    'source(s), judge that fact once and apply the SAME outcome to every',
    'restatement — do not validate a bundled claim that restates a fact you',
    'would otherwise reject on its own (e.g. undated Tier-C source). Split a',
    'bundle if only part of it is independently well-supported.',
    '',
