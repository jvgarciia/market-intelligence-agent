 ...
 *
 * Provider interface:
 *   generate({ systemPrompt, userMessage, timeoutMs?, model?, tools? }, { spawnFn? })
 ...
 *
 * Uses `--output-format json` (not the default text mode) so spawnFn's stdout
 * is the CLI's JSON result envelope, not the raw model answer — this is what
 * exposes real usage.server_tool_use counts (actual search/fetch calls made)
 * instead of a hardcoded 'not_available'. The model's actual answer lives in
 * envelope.result and is unwrapped before structured-output extraction.
 *
 * `tools` restricts which CLI tools the model may use (default: WebSearch +
 * WebFetch, unchanged for Stages 01-04). Stage 05's contact-finding passes
 * ['WebSearch'] only — no WebFetch — so the CLI cannot fetch any URL,
 * including a LinkedIn profile page, structurally enforcing the CONTEXT.md
 * rule against visiting one directly rather than relying on the prompt alone.
 *
...
const DEFAULT_TIMEOUT_MS = 540_000; // 9 minutes — Stage 01's organisation-scale search query pushed it past 360s on a real run
 ...
 *   tools         array   — optional; defaults to ['WebSearch', 'WebFetch']
 *
...
export async function generate(params, _injected = {}) {
  const { systemPrompt, userMessage, timeoutMs: paramTimeout, model: paramModel, tools: paramTools } = params;
  ...
  const tools = paramTools || ['WebSearch', 'WebFetch'];
  ...
  const args = [
    '--print',
    '--output-format', 'json', // exposes usage.server_tool_use (real search/fetch counts) and num_turns — text mode cannot
    '--model', model,
  ...
  // --allowedTools ensures the permitted tools run non-interactively.
  args.push('--tools', toolsValue);
  args.push('--allowedTools', toolsValue);
  ...
  // The envelope wraps the model's actual answer in `result`, alongside real
  // usage/tool-call metadata that text-mode output cannot expose at all.

  let envelope;
  try {
    envelope = JSON.parse((spawnResult.stdout || '').trim());
  } catch (e) {
    throw new Error(
      `local-cli provider: could not parse --output-format json envelope: ${e.message}\n` +
      `stdout preview: ${(spawnResult.stdout || '').slice(0, 500)}`
    );
  }

  if (envelope.is_error) {
    throw new Error(`local-cli provider: CLI reported an error result: ${JSON.stringify(envelope).slice(0, 500)}`);
  }
  ...
  // usage.server_tool_use.web_search_requests is always 0 for the CLI's own
  // WebSearch tool — verified empirically against real search calls. The
  // real count lives in modelUsage: Claude Code routes WebSearch through a
  // cheaper sub-agent model, and that model's usage entry has the true
  // webSearchRequests figure. Summed across all models in case more than one
  // was used. WebFetch has no equivalent field anywhere in the envelope —
  // stays 'not_available' rather than reporting a number that's silently
  // always zero.
  const webSearchRequests = Object.values(envelope.modelUsage || {})
    .reduce((sum, m) => sum + (typeof m.webSearchRequests === 'number' ? m.webSearchRequests : 0), 0);
  ...
  return {
    ...
    latencyMs,
    usage: {
      inputTokens: envelope.usage?.input_tokens ?? 'not_available',
      outputTokens: envelope.usage?.output_tokens ?? 'not_available',
    },
    // Real counts from the CLI's own usage report — lets a low contact/signal
    // yield be diagnosed (e.g. "0 searches ran" vs. "12 searches, nothing found")
    // without re-running the stage or guessing from latency alone.
    toolActivity: {
      webSearchRequests: envelope.modelUsage ? webSearchRequests : 'not_available',
      webFetchRequests: 'not_available', // no field in the CLI envelope exposes this, verified empirically
      numTurns: envelope.num_turns ?? 'not_available',
    },
    warnings,
    rawProviderOutput: {
