 ...
 */
export function buildUserMessage(request, stageCtx, candidates, validated, scores, runId, createdAt) {
  ...
  return [
    ...
    stageCtx.contract,
    '',
    '## 2. Upstream Input — write a brief for each strong candidate',
 ...
 * @returns {{ briefs, meta }}
 */
export async function runStage05(request, candidates, validated, scores, provider) {
  const createdAt = new Date().toISOString();
  const stageCtx = loadStageContext();

  const systemPrompt = buildSystemPrompt();
  const userMessage = buildUserMessage(request, stageCtx, candidates, validated, scores, request.runId, createdAt);
