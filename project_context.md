...
**Current phase:** Phase 3 → Phase 9 transition. **V3.11 is current.** All five workflow stages (market-signals → candidate-discovery → evidence-validation → opportunity-scoring → opportunity-brief) have live execution paths, gated on the appropriate approvals, and have each run successfully against real Italian water-utility data at both regional (Lombardia/Lazio, Sicilia, Calabria, Trentino) and national scale. Both Review Gates now have a real CLI (`scripts/review-gate.mjs`, `npm run workflow:review2`) — no more hand-edited JSON for gate 1/gate 2 decisions. `contacts-master.md` consolidates contacts from every Gate-2-approved run: **20/30 toward the contact target**, from 5 approved runs (regional `...736be5a4`, national `...cfa09872`, Sicilia `...b5971aea`, Calabria `...705731b4`, Trentino `...a7c6658d`). `ASSUMPTIONS-TO-REVISIT.md` now tracks every assignment-specific decision made instead of a genuine production one, for revisiting once real HULO input exists. 217 tests passing (10 eval cases). All work through this point is pushed to `origin/main`. Full session-by-session detail is in Project History below (2026-06-13 through 2026-07-26 entries).
