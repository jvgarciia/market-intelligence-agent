...
- Contact target: 17/30 (as of the Calabria + Trentino runs). Growing this further means either running more regions/markets through the pipeline or improving Stage 05's contact hit rate — Calabria and Trentino both landed only 0-1 contacts each, well below the earlier Sicilia run's 5, so the next region isn't guaranteed to move the needle much; may be worth revisiting Stage 05's hit rate itself as a lever instead of pure region count.
