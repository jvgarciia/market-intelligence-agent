...
**Current phase:** Phase 3 → Phase 9 transition. **V3.10 is current.** All five workflow stages (market-signals → candidate-discovery → evidence-validation → opportunity-scoring → opportunity-brief) have live execution paths, gated on the appropriate approvals, and have each run successfully against real Italian water-utility data at both regional (Lombardia/Lazio) and national scale. Both Review Gates now have a real CLI (`scripts/review-gate.mjs`, `npm run workflow:review2`) — no more hand-edited JSON for gate 1/gate 2 decisions. `contacts-master.md` consolidates contacts from every Gate-2-approved run: **12/30 toward the contact target**, from 2 approved runs (regional `...736be5a4`, national `...cfa09872`). `ASSUMPTIONS-TO-REVISIT.md` now tracks every assignment-specific decision made instead of a genuine production one, for revisiting once real HULO input exists. 217 tests passing. All work through this point is pushed to `origin/main`. Full session-by-session detail is in Project History below (2026-06-13 through 2026-07-24 entries).
...
- Contact target: 12/30. Growing this further means either running more regions/markets through the pipeline or improving Stage 05's contact hit rate. Current lean is region-specific eval cases (Sicilia, Calabria, Trentino — regions the national run only lightly touched) over rerunning the national case, since a rerun of the same query tends to converge on the same candidates rather than surface new ones.
