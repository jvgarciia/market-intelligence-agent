...
**Gate 2 re-approved** for the national run's `--brief-all` version (14 briefs, 11 contacts) — supersedes the earlier curated-version approval; that content stays backed up at `gate2-approved-curated-backup/` but is no longer the active reviewed content for this run. `contacts-master.md` regenerated: **12/30 contacts** (11 from the national run's brief-all pass + 1 from the regional run). The two contacts from the now-superseded curated version (Paolo Lanza, Daniele Granato) correctly dropped out, since they're no longer part of that run's active, approved content — exactly the self-healing behavior the earlier bugfix was for.
